...
as $$
  select jsonb_build_object(
    ...
    'dates', coalesce((
      select jsonb_agg(to_jsonb(d) order by d.votes desc, d.value)
      from (
        select preferred_date::text as value, count(*) as votes
    ...
    'times', coalesce((
      select jsonb_agg(to_jsonb(t) order by t.votes desc, t.value)
      from (
        select to_char(preferred_time, 'HH24:MI') as value, count(*) as votes
        from public.karaoke_registrations
    ...
    'venues', coalesce((
      select jsonb_agg(to_jsonb(v) order by v.votes desc, v.value)
      from (
        select preferred_venue as value, count(*) as votes
        from public.karaoke_registrations
        group by preferred_venue
        order by votes desc, value
        limit 5
      ) v
    ), '[]'::jsonb)
