...
const VENUES = [
  { region: "Selangor", name: "Karaoke Manekineko NU Empire", note: "Kebanyakan pakej menyediakan free-flow teh, kopi dan minuman ringan tertentu." },
  { region: "Selangor", name: "Karaoke Manekineko Central i-City", note: "Free-flow drinks tersedia dan sesuai untuk kumpulan kecil atau besar." },
  { region: "Selangor", name: "Red Box Karaoke The Curve", note: "Pakej refill atau free-flow bergantung pada promosi dan waktu; happy hour biasanya lebih berbaloi." },
  { region: "Selangor", name: "Loud Speaker SetiaWalk Puchong", note: "Pakej karaoke dengan minuman tanpa had ditawarkan pada waktu tertentu." },
  { region: "Selangor", name: "The Port Family Karaoke @ IOI Mall", note: "Beberapa pakej termasuk refill minuman dan sesuai untuk keluarga." },
  { region: "Selangor", name: "K Song Family Karaoke ÆON Mall Shah Alam", note: "Pilihan harga berpatutan dengan promosi minuman refill tertentu." },
  { region: "Selangor", name: "K-Aladdin Family Karaoke Ole Ole Mall", note: "Pakej minuman refill tersedia untuk tempahan bilik tertentu." },
  { region: "Putrajaya", name: "Red Box Karaoke IOI City Mall", note: "Pilihan berhampiran Putrajaya dengan pakej free-flow, terutama waktu siang atau happy hour." }
];
...
function venueOptions(current = "") {
  const groups = ["Selangor", "Putrajaya"];
  let options = `<option value="">Pilih tempat karaoke</option>`;
  if (current && !VENUES.some((venue) => venue.name === current)) options += `<optgroup label="Pilihan sedia ada"><option value="${escapeHtml(current)}" selected>${escapeHtml(current)}</option></optgroup>`;
  for (const group of groups) {
    options += `<optgroup label="${group}">${VENUES.filter((venue) => venue.region === group).map((venue) => `<option value="${escapeHtml(venue.name)}" ${venue.name === current ? "selected" : ""}>${escapeHtml(venue.name)}</option>`).join("")}</optgroup>`;
  }
  return options;
}

function venueNote(name) {
  return VENUES.find((venue) => venue.name === name)?.note || (name ? "Pilihan lokasi sedia ada anda." : "Pilih lokasi untuk melihat maklumat pakej minuman.");
}

function bindVenueHelper(selectId, noteId) {
  const select = document.querySelector(`#${selectId}`);
  const note = document.querySelector(`#${noteId}`);
  if (!select || !note) return;
  const update = () => { note.textContent = venueNote(select.value); };
  select.addEventListener("change", update); update();
}

function suggestionList(items, formatter = (value) => value) {
  if (!items?.length) return `<div class="no-suggestions">Belum ada cadangan</div>`;
  const maximum = Math.max(...items.map((item) => Number(item.votes)));
  return items.map((item, index) => `<div class="suggestion-row"><span class="rank">${index + 1}</span><div class="suggestion-label"><strong>${escapeHtml(formatter(item.value))}</strong><i style="width:${Math.max(12, (Number(item.votes) / maximum) * 100)}%"></i></div><b>${item.votes} undi</b></div>`).join("");
}

function publicDashboard(summary = {}) {
  const total = Number(summary.total_registered || 0);
  const attending = Number(summary.attending || 0);
  const notAttending = Number(summary.not_attending || 0);
  return `<section class="public-dashboard"><div class="public-dashboard-head"><div><p class="step">LIVE COMMUNITY DASHBOARD</p><h2>Suara komuniti kita</h2><p>Keputusan berubah secara langsung apabila peserta mengemas kini pilihan.</p></div><span class="live-data"><i></i> LIVE</span></div><div class="public-metrics"><article><span class="metric-icon purple">●</span><div><small>SUDAH DAFTAR</small><strong>${total}</strong><p>orang peserta</p></div></article><article><span class="metric-icon green">✓</span><div><small>AKAN HADIR</small><strong>${attending}</strong><p>${total ? Math.round((attending / total) * 100) : 0}% daripada pendaftaran</p></div></article><article><span class="metric-icon pink">×</span><div><small>TIDAK HADIR</small><strong>${notAttending}</strong><p>${total ? Math.round((notAttending / total) * 100) : 0}% daripada pendaftaran</p></div></article></div><div class="suggestion-board"><article><div class="suggestion-title"><span>📅</span><div><small>CADANGAN</small><h3>Tarikh pilihan</h3></div></div>${suggestionList(summary.dates, formatDate)}</article><article><div class="suggestion-title"><span>⏰</span><div><small>CADANGAN</small><h3>Masa pilihan</h3></div></div>${suggestionList(summary.times, (value) => value)}</article><article><div class="suggestion-title"><span>📍</span><div><small>CADANGAN</small><h3>Lokasi pilihan</h3></div></div>${suggestionList(summary.venues, (value) => value)}</article></div></section>`;
}

async function loadPublicDashboard() {
  const container = document.querySelector("#publicDashboard");
  if (!container) return;
  try { container.innerHTML = publicDashboard(await rpc("get_karaoke_public_summary", {})); }
  catch { container.innerHTML = `<section class="public-dashboard dashboard-error"><p>Dashboard belum tersedia. Sila cuba sebentar lagi.</p></section>`; }
}
...
function renderLogin() {
  document.querySelector("#app").innerHTML = shell(`<section class="hero login-layout"><div class="hero-copy"><div class="live-badge"><span></span> KARAOKE · DISCO · GOOD VIBES</div><p class="eyebrow">PILIH TARIKH · MASA · TEMPAT</p><h1>Own the<br><em>spotlight.</em></h1><p class="lead">Satu pentas. Seribu memori. Daftar kehadiran dan pilih lagu untuk malam paling meriah tahun ini.</p><div class="countdown"><span class="pulse"></span>Cadangan anda menentukan acara kita</div><div class="equalizer" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i><i></i><i></i><i></i></div></div>
    ...
    <section class="event-strip"><div><small>TARIKH</small><strong>Pilihan bersama</strong></div><div><small>MASA</small><strong>Anda cadangkan</strong></div><div><small>TEMPAT</small><strong>Lokasi pilihan staf</strong></div></section><div id="publicDashboard">${publicDashboard()}</div>`);
  loadPublicDashboard();
...
function bindDashboard(person) {
  bindVenueHelper("dashboardVenue", "dashboardVenueNote");
