...
function shell(content) {
  return `
    ...
    <header class="topbar">
      ...
      ${staffId ? `<button class="ghost" id="logoutBtn">Keluar</button>` : `<span class="date-chip">ANDA PILIH · KITA UNDI</span>`}
    </header>
    <main>${content}</main>
  ...
  document.querySelector("#app").innerHTML = shell(`
    <section class="hero login-layout">
      <div class="hero-copy">
        ...
        <p class="eyebrow">PILIH TARIKH · MASA · TEMPAT</p>
        ...
        <div class="countdown"><span class="pulse"></span>Cadangan anda menentukan acara kita</div>
    ...
    <section class="event-strip">
      <div><small>TARIKH</small><strong>Pilihan bersama</strong></div>
      <div><small>MASA</small><strong>Anda cadangkan</strong></div>
      <div><small>TEMPAT</small><strong>Lokasi pilihan staf</strong></div>
    </section>`);
...
function registrationForm() {
  return `<section class="portal-head"><p class="eyebrow">SELAMAT DATANG</p><h1>Jom rancang <em>malam kita.</em></h1><p>Daftar kehadiran dan cadangkan tarikh, masa serta tempat yang paling sesuai.</p></section>
    <section class="form-card card narrow">
      ...
      <form id="registerForm">
        ...
        <div class="preference-block"><div class="preference-heading"><span>02</span><div><h3>Cadangan acara anda</h3><p>Pilih waktu dan lokasi yang paling sesuai.</p></div></div>
          <div class="preference-grid"><div><label for="preferredDate">Tarikh pilihan</label><input id="preferredDate" name="preferredDate" type="date" required /></div><div><label for="preferredTime">Masa pilihan</label><input id="preferredTime" name="preferredTime" type="time" required /></div><div class="venue-field"><label for="preferredVenue">Tempat pilihan</label><input id="preferredVenue" name="preferredVenue" placeholder="Contoh: Dewan Serbaguna" maxlength="120" required /></div></div>
        </div>
...
function bindRegistration() {
  document.querySelector("#registerForm").addEventListener("submit", (event) => {
    ...
    data.registrations.push({ staffId, fullName: form.get("fullName").trim(), attendance: form.get("attendance"), preferredDate: form.get("preferredDate"), preferredTime: form.get("preferredTime"), preferredVenue: form.get("preferredVenue").trim(), songs: [], updatedAt: new Date().toISOString() });
...
function dashboard(person) {
  ...
  return `<section class="dashboard-head"><div><p class="eyebrow">RUANG PESERTA</p><h1>Hai, ${escapeHtml(person.fullName.split(" ")[0])}! <span>👋</span></h1><p>Pilihan acara dan senarai lagu boleh dikemas kini pada bila-bila masa.</p></div><div class="status ${attending ? "yes" : "no"}"><span></span>${attending ? "Kehadiran disahkan" : "Tidak hadir"}</div></section>
